...
// WARNING: order matters
	...
	private struct Metrics {
		...
		std::map<S, Credit<CheckBoxlet, S>*> checks;
		std::map<S, Credit<Buttonlet, S>*> buttons;
		float max_width;
	};
	...
	public:
		Settings(PLCMaster* device) : ISatellite(default_logging_level, __MODULE__), gapsize(normal_font_size), device(device) {
			...
			this->settings_ao_style = make_highlight_dimension_style(large_font_size, 8U, 1);
			this->settings_do_style.precision = 0;
	...
	public:
		...
		void on_digital_input(long long timepoint_ms, const uint8* DB4, size_t count4, const uint8* DB205, size_t count205, Syslog* logger) override {
			this->DI_settings(DB205, this->do_metrics, DO_settings_Feedback);
			this->DI_button_settings(DB205, this->psdp_metrics, DI_drag_pipes_settings,true);
			this->DI_button_settings(DB205, this->sbdp_metrics, DI_drag_pipes_settings,false);
		}
	...
	public:
		void load(CanvasCreateResourcesReason reason, float width, float height) override {
			...
			this->load_settings(Mode::DoSettings, this->do_metrics, caption_width, caption_height);
		}
		...
		void reflow(float width, float height) override {
			...
			this->reflow_ao_settings(Mode::psOffset, this->pst_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::psIntermediate, this->psi_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::psDragHead, this->psh_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::sbOffset, this->sbt_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::sbIntermediate, this->sbi_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::sbDragHead, this->sbh_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::psDragPipes, this->psdp_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::sbDragPipes, this->sbdp_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::psDoors, this->psds_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::sbDoors, this->sbds_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::ShoreDischarge, this->shore_discharge_metrics, caption_rx, caption_y);
			this->reflow_ao_settings(Mode::Others, this->other_metrics, caption_rx, caption_y);
			this->reflow_do_settings(Mode::DoSettings, this->do_metrics, caption_rx, caption_y);
	...
	public:
		...
		void on_tap_selected(IGraphlet* g, float local_x, float local_y) override {
			...
			auto command = dynamic_cast<Credit<Buttonlet, DragPipesSettings>*>(g);
			if (tab != nullptr) {
				this->change_mode(1U << _I(tab->id));
			}
			else if (command != nullptr && (this->device->get_mode() != TCPMode::User)) {
				for (auto it = this->tabs.begin(); it != this->tabs.end(); it++) {
					if (it->second->get_state() == ButtonState::Executing) {
						this->device->send_command(DO_drag_pipes_settings(command->id, it->second->id == Mode::psDragPipes ? true : false));
					}
				}
				int a = 1;
			}
		...
		void on_focus(IGraphlet* g, bool yes) override {
			if (yes) {
				...
				auto check = dynamic_cast<Credit<CheckBoxlet, DOSettings>*>(g);	

				if ((editor != nullptr) && (this->device->get_mode() != TCPMode::User)) {
					this->show_virtual_keyboard(ScreenKeyboard::Numpad);
				}
				else if ((check != nullptr) && (this->device->get_mode() != TCPMode::User)) {
					this->device->send_command(DO_settings(this->cid<DOSettings>(check)));
				}
			}
		}
		...
		void load_settings(Mode m, Metrics<S>& ms, float width, float height, unsigned int subcolor = 0xF8F8FFU) {
			...
			for (S id = _E(S, 0); id < S::_; id++) {
				...
				if (lbl_cpt[0] == L'_') {
					...
					if (m == Mode::DoSettings) {
						ms.labels[id] = this->insert_one(new Credit<Labellet, S>(_speak(id), this->setting_font), id);
						ms.checks[id] = this->insert_one(new Credit<CheckBoxlet, S>(CheckState::Default), id);
						ms.checks[id]->fill_extent(0.0F, 0.0F, &editor_width);
					}
					else if (m == Mode::psDragPipes || m == Mode::sbDragPipes) {
						if (id >= _E(S, DragPipesSettings::OffsetTopCheck) && id < S::_) {
							ms.buttons[id] = this->insert_one(new Credit<Buttonlet, S>(_speak(id), width, height), id);
						}
						else {
							ms.labels[id] = this->insert_one(new Credit<Labellet, S>(_speak(id), this->setting_font), id);
							ms.editors[id] = this->insert_one(new Credit<Dimensionlet, S>(DimensionState::Input, this->settings_ao_style, editor_unit(lbl_cpt)), id);
	...
	private:
		template<typename S>
		void reflow_ao_settings(Mode m, Metrics<S>& ms, float rx, float y) {
			...
			for (S id = _E(S, 0); id < S::_; id++) {
				if (m == Mode::psDragPipes|| m == Mode::sbDragPipes) {
					if (id >= _E(S, DragPipesSettings::OffsetTopCheck) && id < S::_) {
						ms.boxes[id]->fill_extent(0.0F, 0.0F, nullptr, &setting_height);
						this->move_to(ms.boxes[id], rx, y, GraphletAnchor::RT);
						//this->move_to(ms.buttons[id], ms.boxes[id], GraphletAnchor::RC, GraphletAnchor::LC, -(inset + ms.max_width));
						this->move_to(ms.buttons[id], ms.boxes[id], GraphletAnchor::CC, GraphletAnchor::CC);
					}
					else {
		...
		template<typename S>
		void reflow_do_settings(Mode m, Metrics<S>& ms, float rx, float y) {
			float caption_width, caption_height, setting_height;
			float inset = this->gapsize * 0.42F;
			unsigned int idx = (1U << _I(m));
			...
			y += (caption_height + inset);
			for (S id = _E(S, 0); id < S::_; id++) {
				if (ms.checks.find(id) == ms.checks.end()) {
					ms.labels[id]->fill_extent(0.0F, 0.0F, nullptr, &setting_height);
					this->move_to(ms.labels[id], rx - caption_width * 0.5F, y, GraphletAnchor::CT);
				}
				else {
					...
					this->move_to(ms.boxes[id], rx, y, GraphletAnchor::RT);
					this->move_to(ms.labels[id], ms.boxes[id], GraphletAnchor::LC, GraphletAnchor::LC, inset);
					this->move_to(ms.checks[id], ms.boxes[id], GraphletAnchor::RC, GraphletAnchor::LC, -(inset + ms.max_width));
				}
	...
	private:
		...
		void DI_settings(const uint8* db205, Metrics<S>& ms, F DI) {
			for (S id = _E(S, 0); id < S::_; id++) {
				unsigned int idx = DI(id);

				if (DBX(db205, idx - 1)) {
					ms.checks[id]->set_state(CheckState::Checked);
				}
				else {
					ms.checks[id]->set_state(CheckState::Default);
				}
			}
		}
		template<typename S, typename F>
		void DI_button_settings(const uint8* db205, Metrics<S>& ms, F DI,bool ps) {
			for (S id = _E(S, 0); id < S::_; id++) {
				unsigned int idx = DI(id,ps);
				if (idx > 0) {
					if (DBX(db205, idx - 1)) {
						ms.buttons[id]->set_state(ButtonState::Executing);
					}
					else {
						ms.buttons[id]->set_state(ButtonState::Default);
					}
				}
			}
		}
		...
		template<typename S>
		S cid(CheckBoxlet* check) {
			return dynamic_cast<Credit<CheckBoxlet, S>*>(check)->id;
		}
	...
	private:
		...
		DimensionStyle settings_ao_style;
		CheckStyle settings_do_style;
