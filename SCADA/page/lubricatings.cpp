...
#include "ch13800m3/configuration.hpp"
#include "menu.hpp"
...
#include "ch13800m3/iotables/ai_pumps.hpp"

#include "ch13800m3/iotables/di_pumps.hpp"
#include "ch13800m3/iotables/di_hopper_pumps.hpp"

#include "ch13800m3/iotables/do_hopper_pumps.hpp"
...
public:
	void load(float x, float width, float height, float vinset) {
		...
		//背景灰色和线
		this->unit = this->master->insert_one(new RoundedRectanglet(region_width, region_height, corner_radius, region_background));//上方灰色背景
		this->gearbox = this->master->insert_one(new RoundedRectanglet(region_width, region_height, corner_radius, region_background));//下方灰色背景
		this->station = this->master->insert_one(new Tracklet<GearboxLubricator>(turtle, default_pipe_thickness, default_pipe_color));//管路
		
		//文本和泵
		this->load_label(this->captions, LU::Unit, this->color, this->caption_font, true);//这个好像没用
		this->load_label(this->captions, LU::Gearbox, this->color, this->caption_font, true);//下方文本
		this->load_device(this->units, this->ps, pump_radius);//上方文本和泵
		this->load_devices(this->pumps, this->labels, GearboxLubricator::Master, GearboxLubricator::Spare, pump_radius);//下方文本和泵
...
public:
	void reflow(float x, float width, float height, float vinset) {
		...
		this->master->move_to(this->unit, cx, vinset, GraphletAnchor::CT);//背景灰色
		this->master->move_to(this->gearbox, cx, height - vinset, GraphletAnchor::CB);//下方块?
		this->master->move_to(this->units[this->ps], this->unit, 0.5F, 0.24F, GraphletAnchor::CC);//左泥泵轴承润滑单元
		this->master->move_to(this->station, this->gearbox, 0.5F, 0.46F, GraphletAnchor::CC);//主备泵和管路 此处屏蔽之后报警上移

		this->master->move_to(this->captions[LU::Unit], this->units[this->ps], GraphletAnchor::CT, GraphletAnchor::CB, 0.0F, -gapsize);//左右泥泵润滑单元文本
		this->master->move_to(this->captions[LU::Gearbox], this->station, GraphletAnchor::CT, GraphletAnchor::CB, 0.0F, -gapsize);//左右泥泵齿轮箱润滑油泵文本

		this->station->map_credit_graphlet(this->pumps[GearboxLubricator::Master], GraphletAnchor::CC);//主泵图源
		this->station->map_credit_graphlet(this->pumps[GearboxLubricator::Spare], GraphletAnchor::CC);//副泵图源

		this->master->move_to(this->labels[GearboxLubricator::Master], this->pumps[GearboxLubricator::Master],
			GraphletAnchor::CT, GraphletAnchor::RB, -gapsize);//主 文本 坐标依附图源
		
		this->master->move_to(this->labels[GearboxLubricator::Spare], this->pumps[GearboxLubricator::Spare],
			GraphletAnchor::CT, GraphletAnchor::LB, +gapsize);//副 文本 坐标依附图源
		...
		{ // reflow alarm boxes
			...
			this->master->move_to(this->boxes[LU::Level], cx, alarm_cy, GraphletAnchor::RB, -gapsize, -gapsize * 0.5F);//压力低 轴1#温度高 轴2#温度高 轴3#温度高位置
			this->master->move_to(this->boxes[LU::Pressure], this->boxes[LU::Level], GraphletAnchor::LT, GraphletAnchor::LB, 0.0F, -gapsize);//同上?
			this->master->move_to(this->boxes[LU::Oil], cx, alarm_cy, GraphletAnchor::RT, -gapsize, +gapsize * 0.5F);//油温高 水温高 
			this->master->move_to(this->boxes[LU::Water], this->boxes[LU::Oil], GraphletAnchor::LB, GraphletAnchor::LT, 0.0F, +gapsize);//水温高 依附Oil

			this->master->move_to(this->boxes[LU::Bearing1], this->boxes[LU::Pressure], GraphletAnchor::RC, GraphletAnchor::LC, vinset);//轴1
			this->master->move_to(this->boxes[LU::Bearing2], this->boxes[LU::Bearing1], GraphletAnchor::LB, GraphletAnchor::LT, 0.0F, gapsize);//轴2
			this->master->move_to(this->boxes[LU::Bearing3], this->boxes[LU::Bearing2], GraphletAnchor::LB, GraphletAnchor::LT, 0.0F, gapsize);//轴3

			this->master->fill_graphlet_location(this->gearbox, nullptr, &region_bottom, GraphletAnchor::CB);//下方油温低和压力低低/??
			this->master->fill_graphlet_location(this->station, nullptr, &object_bottom, GraphletAnchor::CB);//下方油温低和压力低低/??

			alarm_cy = object_bottom + (region_bottom - object_bottom) * 0.5F;
			this->master->move_to(this->boxes[LU::GearboxOil], cx, alarm_cy, GraphletAnchor::RC, -gapsize);//下方油温低
			this->master->move_to(this->boxes[LU::GearboxPressure], cx, alarm_cy, GraphletAnchor::LC, gapsize);//下方压力低低
		}

		for (LU id = LU::Pressure; id <= LU::GearboxPressure; id++) {
			this->master->move_to(this->alarms[id], this->boxes[id], GraphletAnchor::LC, GraphletAnchor::LC, vinset); //所有报警的文本
			this->master->move_to(this->captions[id], this->alarms[id], GraphletAnchor::RC, GraphletAnchor::LC, vinset);//所有报警的图标
		}
	}
...
void LubricatingsPage::reflow(float width, float height) {
	...
	if ((ps_dashboard != nullptr) && (sb_dashboard != nullptr)) {
		...
		ps_dashboard->reflow(0.0F,       half_width, height, vinset);//左
		//sb_dashboard->reflow(half_width, half_width, height, vinset);
		sb_dashboard->reflow(width, half_width, height, vinset);//右  测试用
	}
}
