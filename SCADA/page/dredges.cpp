...
private enum class DS : unsigned int {
	...
	PSWC, SBWC, PSPF1, PSPF2, SBPF1, SBPF2, PSSIP, SBSIP,
	TideMark, Speed,
...
bool PSHopperPumpMode;
bool SBHopperPumpMode;
...
static void dredges_diagnostics(DredgesPosition position, PLCMaster* plc) {
	DX side = ((position >= DredgesPosition::sbOffset) ? DX::SB : DX::PS);
...
protected:
	...
	void load_percentage(std::map<E, Credit<Percentagelet, E>*>& ps, E id) {
		...
		this->master->cellophane(ps[id], 0.0F);
	...
	template<typename E>
	void load_label(std::map<E, Credit<Labellet, E>*>& ls, E id, ICanvasBrush^ color, bool p) {//插销限位
		if (p) {
			Platform::String^ label = _speak("pinlimit");
			ls[id] = this->master->insert_one(new Credit<Labellet, E>(label, color), id);
		}
	}
	...
	void load_label(std::map<E, Credit<Labellet, E>*>& ls, Platform::String^ label, E id, ICanvasBrush^ color, CanvasTextFormat^ font = nullptr) {
		...
		switch (id) {
		case DS::D003:case DS::D004:case DS::D011:case DS::D012:case DS::D013:case DS::D014:case DS::D015:case DS::D016:case DS::LMOD:
			//this->master->cellophane(ls[id], 0.0F);
			break;
		}
	}
...
protected:
	void set_compensator(DS id, const uint8* db203, unsigned int rd_idx, GraphletAnchor a) {
		float progress = RealData(db203, rd_idx + 1U);
	...
	void set_pinlimit(DredgesPosition id, const uint8* db4, unsigned int idx4, AlarmState hlstate = AlarmState::Alert) {
		this->pinlimit[id]->set_state(DBX(db4, idx4 - 1U), AlarmState::Notice, AlarmState::None);
		if (!DBX(db4, idx4 - 1U)) {
			this->master->cellophane(this->pinlimit[id], 0.0F);//插销限位。
			this->master->cellophane(this->pinlimitlabel[id], 0.0F);//插销限位。
		}
		else {
			this->master->cellophane(this->pinlimit[id], 100);//插销限位。
			this->master->cellophane(this->pinlimitlabel[id], 100);//插销限位。
		}
	}
	...
	void set_drag_metrics(DS id, DS vid, const uint8* db2, const uint8* db203, DragInfo& info, DredgeAddress* address) {
		...
		read_drag_figures(db2, db203, &offset, ujoints, &draghead, &suction_depth, &visor_angle,
			address->drag_position, address->visor_progress,
			info.visor_side_a, info.visor_side_b, info.visor_side_c, info.visor_active_length);
		...
		if (this->dragxzes.find(id) != this->dragxzes.end()) {
			this->dragxzes[id]->set_figure(offset, ujoints, draghead, visor_angle);
		...
		this->degrees[vid]->set_value(visor_angle- this->dragxzes[id]->get_arm_degrees(), GraphletAnchor::LC);
	}
...
protected: // never delete these graphlets manually.
	...
	std::map<DredgesPosition, Credit<Alarmlet, DredgesPosition>*> pinlimit;
	std::map<DredgesPosition, Credit<Labellet, DredgesPosition>*> pinlimitlabel;
...
/*************************************************************************************************////耙臂
private class Dredges final : public IDredgingSystem {//耙臂界面
public:
...
public:
	...
	void on_analog_input(long long timepoint_ms, const uint8* DB2, size_t count2, const uint8* DB203, size_t count203, Syslog* logger) override {
		...
		//this->overflowpipe[DS::LeftOverflow]->set_value(RealData(DB203, left_overflow_pipe_progress));
		//this->overflowpipe[DS::RightOverflow]->set_value(RealData(DB203, right_overflow_pipe_progress));
		this->overflowpipe[DS::LeftOverflow]->set_value(RealData(DB203, left_overflow_pipe_progress));
		this->overflowpipe[DS::RightOverflow]->set_value(RealData(DB203, right_overflow_pipe_progress));
		//this->overflowpipe[DS::LeftOverflow]->set_liquid_height(DBD(DB2, average_hopper_height));
		//this->overflowpipe[DS::RightOverflow]->set_liquid_height(DBD(DB2, average_hopper_height));
		this->overflowpipe[DS::LeftOverflow]->set_liquid_height(DBD(DB2, average_hopper_height));
		this->overflowpipe[DS::RightOverflow]->set_liquid_height(DBD(DB2, average_hopper_height));
		this->lengths[DS::LeftOverflow]->set_value(this->overflowpipe[DS::LeftOverflow]->get_value(), GraphletAnchor::CC);
		this->lengths[DS::RightOverflow]->set_value(this->overflowpipe[DS::RightOverflow]->get_value(), GraphletAnchor::CC);
		...
		if (PSHopperPumpMode) {
			...
			this->set_density_speed(DS::PS, DB203, this->ps_address->water_density_speed);
			this->set_cylinder(DS::PSHPDP, RealData(DB203, this->ps_address->water_discharge_pressure));
			this->set_cylinder(DS::PSHPVP, RealData(DB203, this->ps_address->water_vacuum_pressure));
		}
		if (SBHopperPumpMode) {
			...
			this->set_density_speed(DS::SB, DB203, this->sb_address->water_density_speed);
			this->set_cylinder(DS::SBHPDP, RealData(DB203, this->sb_address->water_discharge_pressure));
			this->set_cylinder(DS::SBHPVP, RealData(DB203, this->sb_address->water_vacuum_pressure));
		}
		...
		{ // set winches metrics
			...
			this->set_winch_metrics(DredgesPosition::psOffset, DB2, psws_idx + 0U, pswl_idx + 0U, GraphletAnchor::LC);
			...
			this->set_winch_metrics(DredgesPosition::sbOffset, DB2, sbws_idx + 0U, sbwl_idx + 0U, GraphletAnchor::RC);
	...
	void on_digital_input(long long timepoint_ms, const uint8* DB4, size_t count4, const uint8* DB205, size_t count205, Syslog* logger) override {
		...
		/*
		if (isHopperPumpMode(DB4, true)) {
			this->ps_address->density_speed = 188U;
			this->ps_address->discharge_pressure = 253U;//排压
			this->ps_address->vacuum_pressure = 252U;//真空
		}
		else {
			this->ps_address->density_speed = 184U;//密度
			this->ps_address->discharge_pressure = 110U;//排压
			this->ps_address->vacuum_pressure = 108;//真空
		}
		if (isHopperPumpMode(DB4, false)) {
			this->ps_address->density_speed = 190U;
			this->ps_address->discharge_pressure = 257U;//排压
			this->ps_address->vacuum_pressure = 256U;//真空
		}
		else {
			this->ps_address->density_speed = 186U;//密度
			this->ps_address->discharge_pressure = 122U;//排压
			this->ps_address->vacuum_pressure = 120U;//真空
		}*/
		PSHopperPumpMode = HopperPumpMode(DB4, true);
		SBHopperPumpMode = HopperPumpMode(DB4, false);
		...
		//!!DI_gate_valve(this->valves[DS::D003], DB205, gate_valve_HV01_feedback, DB205, gate_valve_HV01_status);
		//!!DI_gate_valve(this->valves[DS::D004], DB205, gate_valve_HV02_feedback, DB205, gate_valve_HV02_status);
		DI_gate_valve(this->valves[DS::D011], DB4, gate_valve_HV11_feedback, DB205, gate_valve_HV11_status);
		DI_gate_valve(this->valves[DS::D012], DB4, gate_valve_HV12_feedback, DB205, gate_valve_HV12_status);
		DI_gate_valve(this->valves[DS::D013], DB4, gate_valve_HV13_feedback, DB205, gate_valve_HV13_status);
		//!!DI_gate_valve(this->valves[DS::D014], DB4, gate_valve_HV14_feedback, DB205, gate_valve_HV14_status);
		DI_gate_valve(this->valves[DS::D015], DB4, gate_valve_HV15_feedback, DB205, gate_valve_HV15_status);
		DI_gate_valve(this->valves[DS::D016], DB4, gate_valve_HV16_feedback, DB205, gate_valve_HV16_status);

		DI_gantry(this->gantries[DredgesPosition::psOffset], DB4, gantry_ps_offset_limited, DB205, gantry_ps_offset_details);
		...
		DI_gantry(this->gantries[DredgesPosition::sbOffset], DB4, gantry_sb_offset_limited, DB205, gantry_sb_offset_details);
		...
		DI_winch(this->winches[DredgesPosition::psOffset], DB4, winch_ps_offset_feedback, winch_ps_offset_limits, DB205, winch_ps_offset_details);
		...
		this->set_winch_limits(DredgesPosition::psOffset, DB4, winch_ps_offset_limits, DB205, winch_ps_offset_details);
		...
		DI_winch(this->winches[DredgesPosition::sbOffset], DB4, winch_sb_offset_feedback, winch_sb_offset_limits, DB205, winch_sb_offset_details);
		...
		this->set_winch_limits(DredgesPosition::sbOffset, DB4, winch_sb_offset_limits, DB205, winch_sb_offset_details);
		...
		this->set_hopper_type(DS::PS, DB4);
		this->set_hopper_type(DS::SB, DB4);

		set_pinlimit(DredgesPosition::psOffset, DB4, ps_offset_pinlimit);//左弯管
		set_pinlimit(DredgesPosition::psIntermediate, DB4, ps_intermediate_pinlimit);//左耙中
		set_pinlimit(DredgesPosition::psDragHead, DB4, ps_dragHead_pinlimit);//左耙头

		set_pinlimit(DredgesPosition::sbOffset, DB4, sb_offset_pinlimit);//右弯管
		set_pinlimit(DredgesPosition::sbIntermediate, DB4, sb_intermediate_pinlimit);//右耙中
		set_pinlimit(DredgesPosition::sbDragHead, DB4, sb_dragHead_pinlimit);//右耙头

		DI_depth_buttons(this->ps_depthcommand[DepthCommand::Encoder], this->ps_depthcommand[DepthCommand::Angle], DB205, depth_ps_buttons);
		DI_depth_buttons(this->sb_depthcommand[DepthCommand::Encoder], this->sb_depthcommand[DepthCommand::Angle], DB205, depth_sb_buttons);
		DI_manual_control(this->ps_manual_control[ManualControlCommand::ManualControl], DB205, ps_manual_control_button);
		DI_manual_control(this->sb_manual_control[ManualControlCommand::ManualControl], DB205, sb_manual_control_button);
	}

	void on_forat(long long timepoint_ms, const uint8* DB20, size_t count, Syslog* logger) override {
		float left_overflow_target_height = DBD(DB20, left_overflow_pipe_target_height);
		float right_overflow_target_height = DBD(DB20, right_overflow_pipe_target_height);

		this->overflowpipe[DS::LeftOverflow]->set_target_height(left_overflow_target_height, (left_overflow_target_height == 0.0F));
		this->overflowpipe[DS::RightOverflow]->set_target_height(right_overflow_target_height, (right_overflow_target_height == 0.0F));
...
public:
	void load_station(float width, float height, float vinset) {
		...
		/*
		pTurtle->jump_right(1.5F)->jump_right(2.5F, DS::D011)->jump_right(3, DS::sb)->jump_up(4, DS::SBHP);
		pTurtle->jump_right(3, DS::D003)->jump_right(2, DS::SB)->jump_back();
		pTurtle->jump_right(3, DS::d1315)->jump_down(4, DS::d13)->jump_left(3, DS::d013)->jump_down(2, DS::D013)->jump_back(DS::d13);
		pTurtle->jump_down(4, DS::d15)->jump_left(3)->jump_down(2, DS::D015)->jump_back(DS::LMOD);

		pTurtle->jump_left(1.5F)->jump_left(2.5F, DS::D012)->jump_left(3, DS::ps)->jump_up(4, DS::PSHP);
		pTurtle->jump_left(3, DS::D004)->jump_left(2, DS::PS)->jump_back();
		pTurtle->jump_left(3, DS::d1416)->jump_down(4, DS::d14)->jump_right(3, DS::d014)->jump_down(2, DS::D014)->jump_back(DS::d14);
		pTurtle->jump_down(4, DS::d16)->jump_right(3)->jump_down(2, DS::D016);*/

		this->station = this->master->insert_one(new Tracklet<DS>(pTurtle, default_pipe_thickness, default_pipe_color));

		this->load_percentage(this->progresses, DS::D003);//百分比
		this->load_percentage(this->progresses, DS::D004);//百分比
		this->load_valves(this->valves, this->labels, DS::D003, DS::D012, vinset, 0.0);//阀
		this->load_valves(this->valves, this->labels, DS::D013, DS::D016, vinset, -90.0);//阀
		//this->load_label(this->labels, DS::LMOD.ToString(), DS::LMOD, Colours::Cyan, this->station_font);//LMOD
		this->load_label(this->hopper_types, _speak(DS::pump), DS::PS, caption_color, this->caption_font);//泵文泵
		this->load_label(this->hopper_types, _speak(DS::pump), DS::SB, caption_color, this->caption_font);//泵文本


		this->lmod = this->master->insert_one(new Arclet(0.0, 360.0, rlmod, rlmod, default_pipe_thickness, Colours::Green));
		this->hpumps[DS::PSHP] = this->master->insert_one(new Credit<HopperPumplet, DS>(+rx, -ry), DS::PSHP);//舱内泵
		this->hpumps[DS::SBHP] = this->master->insert_one(new Credit<HopperPumplet, DS>(-rx, -ry), DS::SBHP);//舱内泵
		this->suctions[DS::PS] = this->master->insert_one(new Credit<Circlelet, DS>(rsct, default_ps_color, default_pipe_thickness), DS::PS);//吸口
		this->suctions[DS::SB] = this->master->insert_one(new Credit<Circlelet, DS>(rsct, default_sb_color, default_pipe_thickness), DS::SB);//吸口
		//隐藏
		this->master->cellophane(hpumps[DS::PSHP], 0.0F);
		this->master->cellophane(hpumps[DS::SBHP], 0.0F);
		this->master->cellophane(suctions[DS::PS], 0.0F);
		this->master->cellophane(suctions[DS::SB], 0.0F);
		this->master->cellophane(station, 0.0F);
		this->master->cellophane(lmod, 0.0F);
	}
	...
	void load_dashboard(float width, float height, float vinset) {
		...
		{ // load dimensions
			...
			//this->overflowpipe[DS::LeftOverflow] = this->master->insert_one(new OverflowPipelet(hopper_height_range, shheight * 0.382F));
			//this->overflowpipe[DS::RightOverflow] = this->master->insert_one(new OverflowPipelet(hopper_height_range, shheight * 0.382F));
			this->load_overflow(this->overflowpipe, DS::LeftOverflow, shheight * 0.382F);
			this->load_overflow(this->overflowpipe, DS::RightOverflow, shheight * 0.382F);
			this->load_dimension(this->lengths, DS::LeftOverflow, "meter");
			this->load_dimension(this->lengths, DS::RightOverflow, "meter");
			...
			this->load_gantries(this->gantries, DredgesPosition::psOffset, DredgesPosition::psDragHead, +gantry_width);
			this->load_gantries(this->gantries, DredgesPosition::sbOffset, DredgesPosition::sbDragHead, -gantry_width);

			this->load_winches(this->winches, DredgesPosition::psOffset, DredgesPosition::psDragHead, winch_width);
			this->load_winches(this->winches, DredgesPosition::sbOffset, DredgesPosition::sbDragHead, winch_width);
			this->load_pinlimit(DredgesPosition::psOffset, DredgesPosition::sbDragHead, dfmeter_height * 0.1F, this->pinlimit, this->pinlimitlabel);//?
		}

		{ // load drags
			float side_drag_width = width * 0.25F;
			float over_drag_height = height * 0.45F;
			...
			this->load_overview_drag(this->dragxys, DS::PS, -over_drag_height, +default_drag_y_interval, 0);//已经反过来了
			this->load_overview_drag(this->dragxys, DS::SB, -over_drag_height, -default_drag_y_interval, 1);//已经反过来了
			this->load_overview_drag(this->dragxys, DS::SBL, over_drag_height, +default_drag_y_interval, 2);

			this->load_sideview_drag(this->dragxzes, DS::PS, +side_drag_width, drag_depth_degrees_max, 0);//已经反过来了
			this->load_sideview_drag(this->dragxzes, DS::SB, -side_drag_width, drag_depth_degrees_max, 1);//已经反过来了
		...
		{ // load buttons
			...
			this->load_buttons(this->sb_manual_control, DS::SB);//角度越控
		}
	}
...
public:
	void reflow_station(float width, float height, float vinset) {
		...
		//以下已更换
		this->master->move_to(this->cylinders[DS::SBHPDP], this->station, GraphletAnchor::LC, GraphletAnchor::RB, 0.0F, vinset * 2.0F);//排压
		this->master->move_to(this->cylinders[DS::SBHPVP], this->cylinders[DS::SBHPDP], GraphletAnchor::LB, GraphletAnchor::RB, -vinset);//真空
		this->master->move_to(this->cylinders[DS::PSHPDP], this->station, GraphletAnchor::RC, GraphletAnchor::LB, 0.0F, vinset * 2.0F);
		this->master->move_to(this->cylinders[DS::PSHPVP], this->cylinders[DS::PSHPDP], GraphletAnchor::RB, GraphletAnchor::LB, +vinset);
	}
	...
	void reflow_dashboard(float width, float height, float vinset) {
		...
		float offset_y, intermediate_y, draghead_y;

		{ // reflow centeral components
			float wc_offset = vinset * 1.5F;

			//this->master->move_to(this->overflowpipe[DS::LeftOverflow], this->station, GraphletAnchor::CC, GraphletAnchor::CT);
			//this->master->move_to(this->overflowpipe[DS::RightOverflow], this->station, GraphletAnchor::CC, GraphletAnchor::CT);

			this->master->move_to(this->overflowpipe[DS::RightOverflow], width * 0.45F, height * 0.5F, GraphletAnchor::CC);
			this->master->move_to(this->overflowpipe[DS::LeftOverflow], width * 0.55F, height * 0.5F, GraphletAnchor::CC);
			this->master->move_to(this->lengths[DS::RightOverflow], this->overflowpipe[DS::RightOverflow], GraphletAnchor::CB, GraphletAnchor::CT, 0.0F, txt_gapsize);
			this->master->move_to(this->lengths[DS::LeftOverflow], this->overflowpipe[DS::LeftOverflow], GraphletAnchor::CB, GraphletAnchor::CT, 0.0F, txt_gapsize);

			this->master->move_to(this->dfmeters[DS::PS], this->hpumps[DS::SBHP], GraphletAnchor::LT, GraphletAnchor::LB, 0.0F, -vinset);//双指针 位置已更换
			this->master->move_to(this->dfmeters[DS::SB], this->hpumps[DS::PSHP], GraphletAnchor::RT, GraphletAnchor::RB, 0.0F, -vinset);//双指针 位置已更换
			this->master->move_to(this->compensators[DS::SBWC], this->cylinders[DS::SBHPVP], GraphletAnchor::LB, GraphletAnchor::RB, -wc_offset);//波浪补偿器位置

			this->master->move_to(this->dragheads[DS::SBVisor], cx, height * 0.85F, GraphletAnchor::RC, -vinset);
			this->master->move_to(this->dragheads[DS::PSVisor], cx, height * 0.85F, GraphletAnchor::LC, +vinset);
			this->master->move_to(this->compensators[DS::PSWC], this->cylinders[DS::PSHPVP], GraphletAnchor::RB, GraphletAnchor::LB, +wc_offset);
		}

		{ // reflow left dredging system
			float dflx, wclx, dhx;

			this->master->fill_graphlet_location(this->dfmeters[DS::SB], &dflx, nullptr, GraphletAnchor::LC);
			this->master->fill_graphlet_location(this->compensators[DS::SBWC], &wclx, nullptr, GraphletAnchor::LC);
			this->master->fill_graphlet_location(this->dragheads[DS::SBVisor], &dhx, nullptr, GraphletAnchor::LC);

			this->master->move_to(this->dragxys[DS::SB], std::fminf(dflx, wclx), cy, GraphletAnchor::RB, -vinset, vinset * 2.0F);
			this->master->move_to(this->dragxzes[DS::SB], dhx - vinset, height, GraphletAnchor::RB);
			...
			{ // reflow gantries
				...
				this->master->fill_graphlet_location(this->dragxys[DS::SB], nullptr, &offset_y, GraphletAnchor::CT);
				...
				this->master->move_to(this->gantries[DredgesPosition::sbDragHead], lx, offset_y, GraphletAnchor::LT, 0.0F, vinset * 5.0F);
				this->master->move_to(this->gantries[DredgesPosition::sbIntermediate], lx, intermediate_y, GraphletAnchor::LC, 0.0F, vinset * 2.0F);
				this->master->move_to(this->gantries[DredgesPosition::sbOffset], lx, draghead_y, GraphletAnchor::LB, 0.0F, 0);

				/*
				this->master->move_to(this->pinlimit[DredgesPosition::psDragHead], this->gantries[DredgesPosition::sbDragHead], GraphletAnchor::LC);
				this->master->move_to(this->pinlimit[DredgesPosition::psIntermediate], this->gantries[DredgesPosition::sbIntermediate], GraphletAnchor::LC);
				this->master->move_to(this->pinlimit[DredgesPosition::psOffset], this->gantries[DredgesPosition::sbOffset], GraphletAnchor::LC);

				this->master->move_to(this->pinlimit[DredgesPosition::sbDragHead], this->gantries[DredgesPosition::sbDragHead], GraphletAnchor::RC);
				this->master->move_to(this->pinlimit[DredgesPosition::sbIntermediate], this->gantries[DredgesPosition::sbIntermediate], GraphletAnchor::RC);
				this->master->move_to(this->pinlimit[DredgesPosition::sbOffset], this->gantries[DredgesPosition::sbOffset], GraphletAnchor::RC);*/
			}
		}
		...
		{ // reflow right dredging system
			...
			this->master->fill_graphlet_location(this->dfmeters[DS::PS], &dfrx, nullptr, GraphletAnchor::RC);
			this->master->fill_graphlet_location(this->compensators[DS::PSWC], &wcrx, nullptr, GraphletAnchor::RC);
			this->master->fill_graphlet_location(this->dragheads[DS::PSVisor], &dhrx, nullptr, GraphletAnchor::RC);

			this->master->move_to(this->dragxys[DS::PS], std::fmaxf(dfrx, wcrx), cy, GraphletAnchor::LB, vinset, vinset * 2.0F);
			this->master->move_to(this->dragxzes[DS::PS], dhrx + vinset, height, GraphletAnchor::LB);

			{ // reflow winches
				float rx = width;

				this->master->fill_graphlet_location(this->dragxys[DS::PS], nullptr, &offset_y, GraphletAnchor::CT);
				...
				this->master->move_to(this->gantries[DredgesPosition::psDragHead], rx, offset_y, GraphletAnchor::RT, 0.0F, vinset * 5.0F);
				this->master->move_to(this->gantries[DredgesPosition::psIntermediate], rx, intermediate_y, GraphletAnchor::RC, 0.0F, vinset * 2.0F);
				this->master->move_to(this->gantries[DredgesPosition::psOffset], rx, draghead_y, GraphletAnchor::RB, 0.0F, 0);
			}
		}
		...
		{ // reflow dimensions and labels
			...
			for (DredgesPosition id = DredgesPosition::psOffset; id <= DredgesPosition::psDragHead; id++) {
				...
				this->master->move_to(this->pinlimit[id], this->winch_lengths[id], GraphletAnchor::LT, GraphletAnchor::RT, -txt_gapsize * 2);//插销限位
				this->master->move_to(this->pinlimitlabel[id], this->pinlimit[id], GraphletAnchor::LC, GraphletAnchor::RC);//插销限位文本
			}

			for (DredgesPosition id = DredgesPosition::sbOffset; id <= DredgesPosition::sbDragHead; id++) {
				this->master->move_to(this->winches[id], this->gantries[id], GraphletAnchor::CC, GraphletAnchor::CC);
				...
				this->reflow_winch_limits(id);
				this->master->move_to(this->pinlimit[id], this->winch_lengths[id], GraphletAnchor::RT, GraphletAnchor::LT, txt_gapsize * 2);//插销限位
				this->master->move_to(this->pinlimitlabel[id], this->pinlimit[id], GraphletAnchor::RC, GraphletAnchor::LC);//插销限位
			}
		...
		*/
		this->master->move_to(this->sb_suctions[SuctionCommand::Inflate], bx, by);
		this->master->move_to(this->sb_suctions[SuctionCommand::Deflate], this->sb_suctions[SuctionCommand::Inflate], GraphletAnchor::RC, GraphletAnchor::LC);
		this->master->move_to(this->sb_manual_control[ManualControlCommand::ManualControl], this->sb_suctions[SuctionCommand::Deflate], GraphletAnchor::CB, GraphletAnchor::CT,0.0F, vinset * 2.5F);//角度越控
		this->master->move_to(this->pressures[DS::SBSIP], this->sb_suctions[SuctionCommand::Deflate], GraphletAnchor::RC, GraphletAnchor::LC, vinset);

		this->master->move_to(this->ps_suctions[SuctionCommand::Deflate], width - bx, by, GraphletAnchor::RT);
		this->master->move_to(this->ps_suctions[SuctionCommand::Inflate], this->ps_suctions[SuctionCommand::Deflate], GraphletAnchor::LC, GraphletAnchor::RC);
		...
		this->master->move_to(this->ps_depthcommand[DepthCommand::Angle], this->ps_depthcommand[DepthCommand::Encoder], GraphletAnchor::RC, GraphletAnchor::LC);

		this->master->move_to(this->sb_depthcommand[DepthCommand::Encoder], this->degrees[DS::SBVisor], GraphletAnchor::LB, GraphletAnchor::CT);
		this->master->move_to(this->sb_depthcommand[DepthCommand::Angle], this->sb_depthcommand[DepthCommand::Encoder], GraphletAnchor::RC, GraphletAnchor::LC);
		}
	}
...
public:
	...
	void on_tap_selected(IGraphlet* g, float local_x, float local_y) {
		...
		auto depth_btn = dynamic_cast<GroupCredit<Buttonlet, DS, DepthCommand>*>(g);
		auto manual_btn = dynamic_cast<GroupCredit<Buttonlet, DS, ManualControlCommand>*>(g);
		...
		else if (depth_btn != nullptr) {
			if (depth_btn->id == DepthCommand::Encoder) {
				plc->send_command((depth_btn->gid == DS::PS) ? drag_ps_depth_command : drag_sb_depth_command,true);
			}
			else if (depth_btn->id == DepthCommand::Angle) {
				plc->send_command((depth_btn->gid == DS::PS) ? drag_ps_depth_command : drag_sb_depth_command,false);
			}
		}else if (manual_btn != nullptr) {
			plc->send_command((manual_btn->gid == DS::PS) ? ps_manual_control_comand : sb_manual_control_comand);
		}
	}
...
private:
	...
	void load_valves(std::map<E, G*>& gs, std::map<E, Credit<Labellet, E>*>& ls, E id0, E idn, float radius, double degrees) {
		for (E id = id0; id <= idn; id++) {
			...
			this->master->cellophane(gs[id], 0.0F);
		}
	}
	...
	template<class G, typename E>
	void load_pinlimit(E id0, E idn, float size, std::map<E, Credit<G, E>*>& cm, std::map<E, Credit<Labellet, E>*>& ls) {
		for (E id = id0; id <= idn; id++) {
			this->load_label(ls, id, Colours::Silver, true);
			cm[id] = this->master->insert_one(new Credit<G, E>(size), id);
			this->master->cellophane(this->pinlimit[id], 0.0F);//插销限位。
			this->master->cellophane(this->pinlimitlabel[id], 0.0F);//插销限位。
		}
	}
...
private:
	...
	void set_hopper_type(DS id, const uint8* db4) {
		if ((PSHopperPumpMode && id == DS::PS) || (SBHopperPumpMode && id == DS::SB)) {
...
private: // never delete these graphlets manually.
	...
	std::map<ManualControlCommand, GroupCredit<Buttonlet, DS, ManualControlCommand>*> ps_manual_control;
	std::map<ManualControlCommand, GroupCredit<Buttonlet, DS, ManualControlCommand>*> sb_manual_control;
	...
	//OverflowPipelet* leftoverflowpipe;
	std::map<DS, Credit<OverflowPipelet, DS>*> overflowpipe;
	std::map<DepthCommand, GroupCredit<Buttonlet, DS, DepthCommand>*> ps_depthcommand;
	std::map<DepthCommand, GroupCredit<Buttonlet, DS, DepthCommand>*> sb_depthcommand;
	Arclet* lmod;
...
public:
	...
	void on_analog_input(long long timepoint_ms, const uint8* DB2, size_t count2, const uint8* DB203, size_t count203, Syslog* logger) override {
		...
		{ // set winches metrics
			...
			if (this->DS_side == DS::PS) {
				this->set_winch_metrics(DredgesPosition::psOffset, DB2, psws_idx + 0U, pswl_idx + 0U, GraphletAnchor::CC);
				...
				this->set_winch_metrics(DredgesPosition::sbOffset, DB2, sbws_idx + 0U, sbwl_idx + 0U, GraphletAnchor::CC);
		...
		if (this->DS_side == DS::PS) {
			this->pump_pressures[DredgesPosition::psOffset]->set_value(RealData(DB203, pump_C1_pressure), GraphletAnchor::CC);
			this->pump_pressures[DredgesPosition::psIntermediate]->set_value(RealData(DB203, pump_B1_pressure), GraphletAnchor::CC);
			this->pump_pressures[DredgesPosition::psDragHead]->set_value(RealData(DB203, pump_A1_pressure), GraphletAnchor::CC);
			...
			this->pressures[DS::PSDP]->set_value(RealData(DB203, this->address->differential_pressure), GraphletAnchor::CC);

			this->set_compensator(DS::PSWC, DB203, this->address->compensator, GraphletAnchor::LC);
			this->Alets[DredgesPosition::psOffset]->set_value(RealData(DB203, winch_ps_offset_A_pressure), GraphletAnchor::LB);
			this->Blets[DredgesPosition::psOffset]->set_value(RealData(DB203, winch_ps_offset_B_pressure), GraphletAnchor::LT);
			...
			if (PSHopperPumpMode && this->DS_side == DS::PS) {
				this->set_density_speed(this->DS_side, DB203, this->address->density_speed);
				this->set_cylinder(DS::PSHPDP, RealData(DB203, this->address->discharge_pressure));
				this->set_cylinder(DS::PSHPVP, RealData(DB203, this->address->vacuum_pressure));
			}
			else {
				this->set_density_speed(this->DS_side, DB203, this->address->water_density_speed);
				this->set_cylinder(DS::PSHPDP, RealData(DB203, this->address->water_discharge_pressure));
				this->set_cylinder(DS::PSHPVP, RealData(DB203, this->address->water_vacuum_pressure));
			}
		}
		else {
			this->pump_pressures[DredgesPosition::sbOffset]->set_value(RealData(DB203, pump_C2_pressure), GraphletAnchor::CC);
			this->pump_pressures[DredgesPosition::sbIntermediate]->set_value(RealData(DB203, pump_B3_pressure), GraphletAnchor::CC);
			this->pump_pressures[DredgesPosition::sbDragHead]->set_value(RealData(DB203, pump_A2_pressure), GraphletAnchor::CC);
			...
			this->pressures[DS::SBDP]->set_value(RealData(DB203, this->address->differential_pressure), GraphletAnchor::CC);

			this->set_compensator(DS::SBWC, DB203, this->address->compensator, GraphletAnchor::RC);

			this->Alets[DredgesPosition::sbOffset]->set_value(RealData(DB203, winch_sb_offset_A_pressure), GraphletAnchor::RB);
			this->Blets[DredgesPosition::sbOffset]->set_value(RealData(DB203, winch_sb_offset_B_pressure), GraphletAnchor::RT);
			...
			if (SBHopperPumpMode && this->DS_side == DS::SB) {
				this->set_density_speed(this->DS_side, DB203, this->address->density_speed);
				this->set_cylinder(DS::SBHPDP, RealData(DB203, this->address->discharge_pressure));
				this->set_cylinder(DS::SBHPVP, RealData(DB203, this->address->vacuum_pressure));
			}
			else {
				this->set_density_speed(this->DS_side, DB203, this->address->water_density_speed);
				this->set_cylinder(DS::SBHPDP, RealData(DB203, this->address->water_discharge_pressure));
				this->set_cylinder(DS::SBHPVP, RealData(DB203, this->address->water_vacuum_pressure));
			}
		}
	...
	void on_digital_input(long long timepoint_ms, const uint8* DB4, size_t count4, const uint8* DB205, size_t count205, Syslog* logger) override {
		if (this->DS_side == DS::PS) {
			DI_hydraulic_pump_dimension(this->pump_pressures[DredgesPosition::psOffset], DB4, pump_C1_feedback);
			DI_hydraulic_pump_dimension(this->pump_pressures[DredgesPosition::psIntermediate], DB4, pump_B1_feedback);
			DI_hydraulic_pump_dimension(this->pump_pressures[DredgesPosition::psDragHead], DB4, pump_A1_feedback);
			...
			DI_boolean_button(this->buttons[DredgesPosition::psOffset], DB205, winch_ps_offset_details.override);
			...
			DI_boolean_button(this->t_gantry_buttons[GantryCommand::VirtualUp], DB205, gantry_ps_offset_virtual_up_limited);
			DI_boolean_button(this->t_gantry_buttons[GantryCommand::VirtualOut], DB205, gantry_ps_offset_virtual_out_limited);
			...
			DI_hydraulic_pump_dimension(this->pump_pressures[DredgesPosition::sbOffset], DB4, pump_C2_feedback);
			DI_hydraulic_pump_dimension(this->pump_pressures[DredgesPosition::sbIntermediate], DB4, pump_B3_feedback);
			DI_hydraulic_pump_dimension(this->pump_pressures[DredgesPosition::sbDragHead], DB4, pump_A1_feedback);
			...
			DI_boolean_button(this->buttons[DredgesPosition::sbOffset], DB205, winch_sb_offset_details.override);
			...
			if (DI_short_sb_drag(DB205)) {
				this->drag_use_buttons[DragUseCommand::ShortDrag]->set_state(ButtonState::Executing);
			}
			else {
				this->drag_use_buttons[DragUseCommand::ShortDrag]->set_state(ButtonState::Default);
			}

			DI_boolean_button(this->t_gantry_buttons[GantryCommand::VirtualUp], DB205, gantry_sb_offset_virtual_up_limited);
			DI_boolean_button(this->t_gantry_buttons[GantryCommand::VirtualOut], DB205, gantry_sb_offset_virtual_out_limited);
...
public:
	void load(float width, float height, float vinset) override {
		...
		this->load_sideview_drag(this->dragxzes, this->DS_side, -side_drag_width * this->sign, drag_depth_degrees_max, this->drag_idx);//侧视图
		this->load_overview_drag(this->dragxys, this->DS_side, -over_drag_height, -default_drag_y_interval * this->sign, this->drag_idx);//俯视图
		...
		//this->load_label(this->labels, DS::Overlook, this->caption_color, this->label_font);//俯视图文本
		//this->load_label(this->labels, DS::Sidelook, this->caption_color, this->label_font);//侧视图文本

		this->load_dimension(this->lengths, DS::TideMark, "meter");
		this->load_dimension(this->speeds, DS::Speed, "knot");

		this->load_setting(this->settings, this->labels, DS::DesignDepth, "meter", 50.0);//加载设计挖深
		this->load_setting(this->settings, DS::TolerantDepth, "meter", 50.0);//
		//this->load_setting(this->settings, this->labels, DS::DivingDepth, "meter", 50.0);//不需要
		//this->load_setting(this->settings, this->labels, DS::DivingCompensation, "centimeter", compensator_range * 100.0);
		//this->load_setting(this->settings, this->labels, DS::LandingDepth, "meter", 50.0);//不需要


		if (this->DS_side == DS::PS) {

			this->load_densityflowmeters(this->dfmeters, DS::PS, DS::PS, side_drag_width * 0.3F);//双指针
			this->load_cylinders(this->cylinders, DS::PSHPDP, DS::PSHPDP, side_drag_width * 0.2F, 0.0, 20.0, "bar");//排压
			this->load_cylinders(this->cylinders, DS::PSHPVP, DS::PSHPVP, side_drag_width * 0.2F, -2.0, 2.0, "bar");//真空
			...
			this->load_buttons(this->t_gantry_buttons, DredgesPosition::psOffset, this->button_style, gvabtn_size, gvabtn_size);
			...
			this->load_densityflowmeters(this->dfmeters, DS::SB, DS::SB, side_drag_width * 0.3F);//双指针
			this->load_cylinders(this->cylinders, DS::SBHPDP, DS::SBHPDP, side_drag_width * 0.2F, 0.0, 20.0, "bar");//排压
			this->load_cylinders(this->cylinders, DS::SBHPVP, DS::SBHPVP, side_drag_width * 0.2F, -2.0, 2.0, "bar");//真空
			...
			this->load_buttons(this->t_gantry_buttons, DredgesPosition::sbOffset, this->button_style, gvabtn_size, gvabtn_size);
			...
			this->load_button(this->drag_use_buttons, DragUseCommand::ShortDrag, DragUseCommand::ShortDrag);
		}
	}
	...
	void reflow(float width, float height, float vinset) override {
		...
		if (this->DS_side == DS::PS) {
			...
			this->master->move_to(this->dragxzes[DS::PS], this->dragxys[DS::PS], GraphletAnchor::RT, GraphletAnchor::LT, vinset);
			{ // reflow gantries and winches
				auto gantry = this->gantries[DredgesPosition::psIntermediate];

				this->pump_pressures[DredgesPosition::psOffset]->fill_extent(0.0F, 0.0F, nullptr, &vgapsize);

				vgapsize = (vgapsize + vinset) * 2.0F;
				this->master->move_to(gantry, this->dragxys[DS::PS], GraphletAnchor::LC, GraphletAnchor::RT, 0.0F, +vinset);////耙中A字架
				this->master->move_to(this->gantries[DredgesPosition::psDragHead], gantry, GraphletAnchor::RT, GraphletAnchor::RB, 0.0F, -vgapsize);//弯管
				this->master->move_to(this->gantries[DredgesPosition::psOffset], gantry, GraphletAnchor::RB, GraphletAnchor::RT, 0.0F, +vgapsize);//耙头

				this->reflow_winches(DS::PSWC, DredgesPosition::psOffset, DredgesPosition::psDragHead, GraphletAnchor::LC, GraphletAnchor::RC, vinset * -2.0F);//绞车
				this->master->move_to(this->dragheads[DS::PSVisor], this->gantries[DredgesPosition::psDragHead], GraphletAnchor::CC, GraphletAnchor::CB, -width * 0.1F, -vgapsize);//耙唇
			}
			...
			this->master->move_to(this->dragxys[DS::SB], cx, cy, GraphletAnchor::CC);//侧视图
			this->master->move_to(this->dragxzes[DS::SB], this->dragxys[DS::SB], GraphletAnchor::LT, GraphletAnchor::RT, -vinset);//俯视图
			...
			this->master->move_to(this->drag_use_buttons[DragUseCommand::ShortDrag], drag_use_buttons[DragUseCommand::LongDrag], GraphletAnchor::CB, GraphletAnchor::CT, 0.0F, vinset);


			{ // reflow gantries and winches
				auto gantry = this->gantries[DredgesPosition::sbIntermediate];

				this->pump_pressures[DredgesPosition::sbOffset]->fill_extent(0.0F, 0.0F, nullptr, &vgapsize);

				vgapsize = (vgapsize + vinset) * 2.0F;
				this->master->move_to(gantry, this->dragxys[DS::SB], GraphletAnchor::RC, GraphletAnchor::LT, 0.0F, +vinset);//耙中A字架
				this->master->move_to(this->gantries[DredgesPosition::sbDragHead], gantry, GraphletAnchor::LT, GraphletAnchor::LB, 0.0F, -vgapsize);//弯管A字架
				this->master->move_to(this->gantries[DredgesPosition::sbOffset], gantry, GraphletAnchor::LB, GraphletAnchor::LT, 0.0F, +vgapsize);//耙头A字架

				this->reflow_winches(DS::SBWC, DredgesPosition::sbOffset, DredgesPosition::sbDragHead, GraphletAnchor::RC, GraphletAnchor::LC, vinset * 2.0F);//绞车坐标
				this->master->move_to(this->dragheads[DS::SBVisor], this->gantries[DredgesPosition::sbDragHead], GraphletAnchor::CC, GraphletAnchor::CB, width * 0.1F, -vgapsize);//耙唇
			}

			for (DredgesPosition id = DredgesPosition::sbOffset; id <= DredgesPosition::sbDragHead; id++) {
				this->master->move_to(this->pump_pressures[id], this->gantries[id], GraphletAnchor::RB, GraphletAnchor::CT, -vinset, txt_gapsize);//泵压力
				this->master->move_to(this->position_labels[id], this->winches[id], GraphletAnchor::LC, GraphletAnchor::RB);//绞车文本
				this->master->move_to(this->Alets[id], this->winches[id], GraphletAnchor::RC, GraphletAnchor::LB, txt_gapsize, -1.0F);//A口压力
				this->master->move_to(this->Blets[id], this->winches[id], GraphletAnchor::RC, GraphletAnchor::LT, txt_gapsize, 1.0F);//B口压力
				this->master->move_to(this->buttons[id], this->Blets[id], GraphletAnchor::CB, GraphletAnchor::CT, 0.0F, 1.0F);//编码器越控按钮
				...
				this->master->move_to(this->winch_speeds[id], this->winch_lengths[id], GraphletAnchor::CB, GraphletAnchor::CT);//绞车速度
			}
		}
		...
		{ // reflow labels, dimensions and settings
			...
			if (this->DS_side == DS::PS) {
				...
				this->master->move_to(this->forces[DS::PSPF1], this->dragxzes[DS::PS], GraphletAnchor::RT, GraphletAnchor::RB, -pf_xoff);
				this->master->move_to(this->forces[DS::PSPF2], this->dragxzes[DS::PS], GraphletAnchor::LT, GraphletAnchor::LB, +pf_xoff);

				this->master->move_to(this->settings[DS::TolerantDepth], this->dragxzes[DS::PS], GraphletAnchor::RB, GraphletAnchor::RT, -vinset, vinset);
				...
				this->master->move_to(this->settings[asetting], this->dragxzes[DS::PS], 1.0F, this->dragheads[DS::PSVisor], 0.36F, GraphletAnchor::RC);
				this->master->move_to(this->labels[asetting], this->settings[asetting], GraphletAnchor::LC, GraphletAnchor::RC, -vinset);

				this->master->move_to(this->dfmeters[DS::PS], this->labels[DS::DesignDepth], GraphletAnchor::CB, GraphletAnchor::CT, vgapsize, vgapsize / 6);//双指针

				//this->master->move_to(this->cylinders[DS::SBHPDP], this->dfmeters[DS::SB], GraphletAnchor::CB, GraphletAnchor::CB, -vgapsize * 5, -vgapsize);//排压
				//this->master->move_to(this->cylinders[DS::SBHPVP], this->cylinders[DS::SBHPDP], GraphletAnchor::RB, GraphletAnchor::LB, -vinset);//真空

				this->master->move_to(this->cylinders[DS::PSHPDP], this->dfmeters[DS::PS], GraphletAnchor::CB, GraphletAnchor::CB, -vgapsize * 3, -vgapsize);//排压
				this->master->move_to(this->cylinders[DS::PSHPVP], this->cylinders[DS::PSHPDP], GraphletAnchor::LB, GraphletAnchor::RB, -vinset);//真空
			}
			else {
				this->reflow_draghead_metrics(DS::SBVisor, DS::SBDP);
				this->master->move_to(this->labels[DS::SBWC], this->compensators[DS::SBWC], GraphletAnchor::CT, GraphletAnchor::CB);//波浪补偿器 文本
				this->master->move_to(this->forces[DS::SBPF1], this->dragxzes[DS::SB], GraphletAnchor::LT, GraphletAnchor::LB, +pf_xoff);//耙管拉力1
				this->master->move_to(this->forces[DS::SBPF2], this->dragxzes[DS::SB], GraphletAnchor::RT, GraphletAnchor::RB, -pf_xoff);//耙管拉力2

				this->master->move_to(this->labels[DS::DesignDepth], this->dragxzes[DS::SB], GraphletAnchor::LB, GraphletAnchor::LT, vinset, vinset);//设计挖深设置文本
				this->master->move_to(this->settings[DS::DesignDepth], this->labels[DS::DesignDepth], GraphletAnchor::RC, GraphletAnchor::LC, vinset);//设计挖深设置
				this->master->move_to(this->settings[DS::TolerantDepth], this->settings[DS::DesignDepth], GraphletAnchor::RC, GraphletAnchor::LC, vinset);//设计挖深设置

				//this->master->move_to(this->labels[asetting], this->dragxzes[DS::PS], 0.0F, this->dragheads[DS::PSVisor], 0.42F, GraphletAnchor::LC);//设置文本
				//this->master->move_to(this->settings[asetting], this->labels[asetting], GraphletAnchor::RC, GraphletAnchor::LC, +vinset);//设置框
				this->master->move_to(this->dfmeters[DS::SB], this->labels[DS::DesignDepth], GraphletAnchor::CB, GraphletAnchor::CT, vgapsize, vgapsize / 4);//双指针
				this->master->move_to(this->cylinders[DS::SBHPDP], this->dfmeters[DS::SB], GraphletAnchor::CB, GraphletAnchor::CB, vgapsize * 5, -vgapsize);//排压
				this->master->move_to(this->cylinders[DS::SBHPVP], this->cylinders[DS::SBHPDP], GraphletAnchor::LB, GraphletAnchor::RB, -vinset);//真空
			}
...
public:
	void draw_cables(CanvasDrawingSession^ ds, float X, float Y, float Width, float Height) {
		...
		if (this->DS_side == DS::PS) {
			this->draw_cable(ds, X, Y, DredgesPosition::psOffset, GraphletAnchor::LT, GraphletAnchor::RC, color, thickness);
			this->draw_cable(ds, X, Y, DredgesPosition::psIntermediate, GraphletAnchor::LT, GraphletAnchor::RC, color, thickness);
			this->draw_cable(ds, X, Y, DS::PSWC, DredgesPosition::psDragHead, GraphletAnchor::LT, GraphletAnchor::RC, color, thickness);
		}
		else {
			this->draw_cable(ds, X, Y, DredgesPosition::sbOffset, GraphletAnchor::RT, GraphletAnchor::LC, color, thickness);
			this->draw_cable(ds, X, Y, DredgesPosition::sbIntermediate, GraphletAnchor::RT, GraphletAnchor::LC, color, thickness);
			this->draw_cable(ds, X, Y, DS::SBWC, DredgesPosition::sbDragHead, GraphletAnchor::RT, GraphletAnchor::LC, color, thickness);
		}
	}
...
public:
	...
	void on_tap_selected(IGraphlet* g, float local_x, float local_y) override {
		...
		else if (long_btn != nullptr) {//使用长耙
			//this->master->get_plc_device()->send_command(DO_use_long_sb_drag_command(long_btn->get_state() == ButtonState::Default));
			this->master->get_plc_device()->send_command(DO_use_long_sb_drag_command(long_btn->id));
		}
	}

	void on_gesture(std::list<float2>& anchors, float x, float y) {
		if (this->DS_side == DS::PS) {
			if (this->gantries_selected(DredgesPosition::psOffset, DredgesPosition::psDragHead, 2)) {
				group_menu_popup(this->ggantries_op, this->master, x, y);
			}
		}
		else {
			if (this->gantries_selected(DredgesPosition::sbOffset, DredgesPosition::sbDragHead, 2)) {
				group_menu_popup(this->ggantries_op, this->master, x, y);
			}
		}
	}
...
private:
	...
	void reflow_winches(D wc, E offset, E draghead, GraphletAnchor ga, GraphletAnchor wa, float gapsize) {
		...
		{ // align gantries and winches
			...
			for (E id = offset; id <= draghead; id++) {
...
private: // never delete these graphlets manually.
	...
	std::map<DS, Credit<DensitySpeedmeterlet, DS>*> dfmeters;//双指针
	std::map<DS, Credit<Cylinderlet, DS>*> cylinders;//柱状图
...
public:
	...
	void reflow(float width, float height, float vinset) override {
		...
		{ // reflow left dredging system
			...
			this->master->move_to(this->winches[DredgesPosition::psOffset], this->dragxzes[DS::PS], GraphletAnchor::RT, GraphletAnchor::RB, -inset);
		...
		{ // reflow right dredging system
			...
			this->master->move_to(this->winches[DredgesPosition::sbOffset], this->dragxzes[DS::SB], GraphletAnchor::LT, GraphletAnchor::LB, +inset);
