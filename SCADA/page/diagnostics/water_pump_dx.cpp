...
public:
	WaterPumpDx(WaterPumpDiagnostics* master, bool ps, unsigned int color) : master(master), ps(ps) {
		...
		this->sc_start = WP::Remote;
		this->sc_end = WP::LeaveThe0;
...
public:
	...
	void on_digital_input(long long timepoint_ms, const uint8* DB4, size_t count4, const uint8* DB205, size_t count205, Syslog* logger) override {
		...
		//unsigned int plready = (this->ps ? ps_water_pump_pipeline_ready : sb_water_pump_pipeline_ready) - 1U;//管系备妥
		//unsigned int ready = (this->ps ? 1009U : 1025U) - 1U;//管系备妥
		...
		this->diagnoses[WP::Remote]->set_state(DBX(DB4, this->ps ? 817U : 849U), AlarmState::Notice, AlarmState::None);
