...
private enum class WG : unsigned int {
	...
	// Hydraulical Pumps
	F1, F2, C2, B4, B3, A2, D1, D2, C1, B2, B1, A1, E, I, J,
...
public:
	DredgesDx(DredgesDiagnostics* master, DX side) : master(master), side(side) {
		...
		if (side == DX::PS) {
			this->sublabel_start = WG::F1;
			this->sublabel_end = WG::D1;

			this->winch_limits = &winch_ps_offset_limits;
			this->winch_details = &winch_ps_offset_details;
		} else {
			this->sublabel_start = WG::D2;
			this->sublabel_end = WG::J;
...
public:
	...
	void on_digital_input(long long timepoint_ms, const uint8* DB4, size_t count4, const uint8* DB205, size_t count205, Syslog* logger) override {
		...
		for (WG id = this->sublabel_start; id <= this->sublabel_end; id++) {
			switch (id) {
			case WG::F1: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_F1_feedback); break;
			case WG::F2: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_F2_feedback); break;
			case WG::C2: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_C2_feedback); break;
			case WG::B4: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_B4_feedback); break;
			case WG::B3: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_B3_feedback); break;
			case WG::A2: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_A2_feedback); break;
			case WG::D1: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_D1_feedback); break;
			case WG::D2: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_D2_feedback); break;
			case WG::C1: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_C1_feedback); break;
			case WG::B2: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_B2_feedback); break;
			case WG::B1: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_B1_feedback); break;
			case WG::A1: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_A1_feedback); break;
			case WG::E: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_E_feedback); break;
			case WG::I: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_I_feedback); break;
			case WG::J: pumps_running &= this->check_hydraulic_pump(id, DB4, pump_J_feedback); break;
			}
		}
