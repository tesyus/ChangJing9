...
protected:
	void construct(CanvasCreateResourcesReason reason) override {
		this->push_planet(new HydraulicsPage(this->device)); // 液压系统
		this->push_planet(new DredgesPage(DragView::_, this->device)); // 耙臂指示
		this->push_planet(new ChargesPage(this->device)); // 装舱|装驳
		this->push_planet(new DischargesPage(this->device)); // 艏吹艏喷
		this->push_planet(new GlandsPage(this->device)); // 泥泵封水管系
		this->push_planet(new FlushsPage(this->device)); // 高压冲水
		this->push_planet(new LubricatingsPage(this->device)); // 泥泵轴承 && 齿轮箱
		this->push_planet(new HopperDoorsPage(this->device)); // 泥门控制
		this->push_planet(new DredgesPage(DragView::PortSide, this->device)); // 左绞车&&吊架
		this->push_planet(new DredgesPage(DragView::Starboard, this->device)); //右绞车&&吊架 
		this->push_planet(new DraughtsPage(this->device)); // 吃水装载
...
int main(Platform::Array<Platform::String^>^ args) {
	return launch_universal_windows_application<ch13800m3>(default_logging_level, remote_test_server);
}
