#include "graphlet/cuttersuction/swingwidthlet.hpp"
...
/*************************************************************************************************/
SwingWidthlet::SwingWidthlet(double range, float width, float height, unsigned int step, unsigned int precision, ICanvasBrush^ color
...
void SwingWidthlet::fill_extent(float x, float y, float* w, float* h) {
	SET_VALUES(w, this->width, h, this->height);
}

void SwingWidthlet::set_liquid_height(double h, bool force) {
...
void SwingWidthlet::set_target_height(double h, bool force) {
...
void SwingWidthlet::on_value_changed(double v) {
...
void SwingWidthlet::on_liquid_height_changed(double h) {
...
void SwingWidthlet::on_target_height_changed(double h) {
	TextExtent te;
	Rect region = this->body->ComputeBounds();
	double percentage = h / (this->vmax - this->vmin);
...
void SwingWidthlet::draw(CanvasDrawingSession^ ds, float x, float y, float Width, float Height) {
	Rect hatchmark_box = this->hatchmark->ComputeBounds();
...
float SwingWidthlet::get_outlet_height(double percentage) {
